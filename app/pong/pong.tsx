...
export function Pong() {
  ...
  // Game constants
  const AI_X = (canvas.current?.clientWidth ?? WIDTH) - PADDLE_WIDTH - 10;
  const ai = {
    x: AI_X,
    y: (canvas.current?.clientHeight ?? HEIGHT) / 2 - PADDLE_HEIGHT / 2,
    width: PADDLE_WIDTH,
  ...
  // Ball object
  const ball = {
    x: (canvas.current?.clientWidth ?? WIDTH) / 2,
    y: (canvas.current?.clientHeight ?? HEIGHT) / 2,
    radius: BALL_RADIUS,
    speed: 5,
